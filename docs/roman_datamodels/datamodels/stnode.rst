...
``roman`` attribute will be a specialized `~roman_datamodels.datamodels.DNode` object rather
...
There are two main container node types `~roman_datamodels.datamodels.DNode` and
`~roman_datamodels.datamodels.LNode` which correspond to "dictionary" and
...
`~roman_datamodels.datamodels.DNode` objects, keys can also be used to directly
access the data attributes of the object via the Python ``.`` operator
(``node.keyword``). This is so that the `~roman_datamodels.datamodels.DNode`
...
stored in a `~roman_datamodels.datamodels.DNode` or `~roman_datamodels.datamodels.LNode`
object, depending on the schema in question.

The specific stnode objects will be subclasses of the
`~roman_datamodels.datamodels.TaggedObjectNode` class
(there is a legacy ``TaggedListNode`` also). These classes are extensions
of the `~roman_datamodels.datamodels.DNode`
