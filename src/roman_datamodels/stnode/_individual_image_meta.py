...
class _SentinelMeta(type):
    """
    Thread safe singleton metaclass for creating sentinel types
    """
...
class _MissingCellType(metaclass=_SentinelMeta):
...
class _SchemaTable:
    """
    Class to extract a row of data from a subtree of a data model node into a table

    Note: This is intended to be built from a schema that is reference by the "individual_schema"
    entry in a RAD schema that defines multiple tables.
    """
...
class _MetaTables:
    ...
    Note: this is intended to be built from a schema that contains multiple table definitions
    with the additional "individual_schema" entry that points to the schema for each table.
    """
    ...
    @classmethod
    def from_rad_schema(cls, uri: str, defaults: dict[str, dict[str, Any] | None] | None = None) -> _MetaTables:
        """
        Create a _MetaTables from a RAD schema that contains multiple table definitions
        ...
        Returns
        -------
            A _MetaTables instance with tables based on the schema
        """
    ...
    def add_image(self, image: DNode):
        """
...
def IndividualImageMeta():
    """
    Generate a _MetaTables instance for blending ImageModel metadata
        This is will target the latest ``individual_image_meta`` schema.

    Returns
    -------
        A _MetaTables instance configured for blending ImageModel metadata
    """
    ...
    return _MetaTables.from_rad_schema(candidate_uri)
