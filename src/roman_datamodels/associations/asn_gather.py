...
def asn_gather(
    ...
    for src_product in source_asn["products"]:
        members = [
            ...
            if src_member["exptype"] not in exclude_types and (exp_types is None or src_member["exptype"] in exp_types)
        ]
        if members:
...
def from_cmdline(args=None):
    ...
    parser.add_argument(
        "-x", "--exclude-types", default=None, dest="exclude_types", action="append", help="Exposure types to exclude."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=("Shell command to use to perform the copy. Specify as a single string. " 'Default: "%(default)s"'),
    )
