...
class LoadAsAssociation(dict):
    ...
    @classmethod
    def load(cls, obj, meta=DEFAULT_ASN_META, registry=AssociationRegistry, rule=Association, product_name_func=None):
        ...
        try:
            ...
            if not isinstance(obj, list):
                obj = [obj]
            asn = asn_from_list(obj, rule=rule, meta=DEFAULT_ASN_META, product_name_func=product_name_func)
    ...
    def load(cls, obj, basename=None):
        ...
        if isinstance(obj, str):
            ...
            if file_ext == ".fits":
                items = [(obj, "science")]
                asn = asn_from_list(
                    items, product_name=file_name, rule=DMSLevel2bBase, with_exptype=True, meta={"asn_pool": "singleton"}
                )
                return asn

        asn = super().load(
            obj,
            registry=AssociationRegistry(definition_files=[libpath("rules_level2b.py")], include_default=False),
            rule=DMSLevel2bBase,
