...
def mkpool(
    ...
    **kwargs,
    ...
    for datum in data:
        ...
        valid_params = {keyword.lower(): str(value) for keyword, value in header.items() if keyword not in IGNORE_KEYS}
...
def from_cmdline(args=None):
    ...
    parser = argparse.ArgumentParser(description="Create an Association Pool file from a list of exposures")

    parser.add_argument("pool", help="Name of the pool file to save to.")
    parser.add_argument("data", nargs="+", help="List of exposures to create the Association Pool with.")

    parser.add_argument("--asn-candidate", default=NON_HEADER_COLS["asn_candidate"], help="Additional candidate information.")
    parser.add_argument(
        "--dms-note", default=NON_HEADER_COLS["dms_note"], help="Added notes that may be relevant to association creation."
    )
    parser.add_argument(
        "--is-imprt", default=NON_HEADER_COLS["is_imprt"], help='A "t" indicates the exposure is an imprint exposure.'
    )
    parser.add_argument("--pntgtype", default=NON_HEADER_COLS["pntgtype"], help="The general class of exposure.")
    parser.add_argument(
