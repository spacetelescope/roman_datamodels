...
__all__ = ("Builder", "FakeDataBuilder", "NodeBuilder", "SchemaType")
...
class Builder:
    ...
    def get_type(self, schema):
        ...
        if any(has_keyword(schema, k) for k in _OBJECT_KEYWORDS):
            return SchemaType.OBJECT
        if any(has_keyword(schema, k) for k in _ARRAY_KEYWORDS):
            return SchemaType.ARRAY
        if any(has_keyword(schema, k) for k in _STRING_KEYWORDS):
            return SchemaType.STRING
        # assume anything numeric is a number not an integer
        if any(has_keyword(schema, k) for k in _NUMERIC_KEYWORDS):
    ...
    def from_object(self, schema, defaults):
        if defaults is NO_VALUE:
            defaults = {}
        obj = {}
        required = get_required(schema)
        if not required:
            return obj
        for name, subschema in get_properties(schema):
        ...
        for name in required:
            subdefaults = defaults.get(name, NO_VALUE)
            for subschema in get_pattern_properties(schema, name):
                if (value := self.build_node(subschema, subdefaults)) is NO_VALUE:
                    continue
                if name in obj and isinstance(value, dict):
                    # blend the 2 dictionaries
    ...
    def from_array(self, schema, defaults):
        ...
        if min_items is MISSING_KEYWORD:
            return arr
        ...
        if items_keyword is MISSING_KEYWORD:
            return arr
...
class FakeDataBuilder(Builder):
    ...
    def from_tagged(self, schema, defaults):
        tag = get_keyword(schema, "tag")
        if tag is MISSING_KEYWORD:
            # FIXME a guidewindow array is missing a tag
            if get_keyword(schema, "ndim"):
    ...
    def make_quantity(self, schema, defaults):
        ...
        props = dict(get_properties(schema))
...
class NodeBuilder(Builder):
    ...
    def from_object(self, schema, defaults):
        ...
        subschemas = dict(get_properties(schema))
        for name, subdefaults in defaults.items():
            if name in subschemas:
                subschema = subschemas[name]
            else:
                subschema = next(get_pattern_properties(schema, name), {})
