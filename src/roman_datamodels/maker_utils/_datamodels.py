...
def mk_level1_science_raw(*, shape=(8, 4096, 4096), filepath=None, **kwargs):
    ...
    wfi_science_raw["data"] = kwargs.get(
        "data", u.Quantity(np.zeros(shape, dtype=np.uint16), u.DN, dtype=np.uint16)
    )
    ...
    wfi_science_raw["amp33"] = kwargs.get(
        "amp33",
        u.Quantity(
            np.zeros((n_groups, 4096, 128), dtype=np.uint16), u.DN, dtype=np.uint16
        ),
    )
...
def mk_level2_image(*, shape=(4088, 4088), n_groups=8, filepath=None, **kwargs):
    ...
    if len(shape) > 2:
        ...
        warnings.warn(
            f"{MESSAGE} assuming the first entry is n_groups followed by y, x. The"
            " remaining is thrown out!",
            UserWarning,
        )
    ...
    wfi_image["border_ref_pix_left"] = kwargs.get(
        "border_ref_pix_left",
        u.Quantity(
            np.zeros((n_groups, shape[0] + 8, 4), dtype=np.float32),
            u.DN,
            dtype=np.float32,
        ),
    )
    ...
    # and their dq arrays
    wfi_image["dq_border_ref_pix_left"] = kwargs.get(
        "dq_border_ref_pix_left", np.zeros((shape[0] + 8, 4), dtype=np.uint32)
    )
    wfi_image["dq_border_ref_pix_right"] = kwargs.get(
        "dq_border_ref_pix_right", np.zeros((shape[0] + 8, 4), dtype=np.uint32)
    )
    wfi_image["dq_border_ref_pix_top"] = kwargs.get(
        "dq_border_ref_pix_top", np.zeros((4, shape[1] + 8), dtype=np.uint32)
    )
    wfi_image["dq_border_ref_pix_bottom"] = kwargs.get(
        "dq_border_ref_pix_bottom", np.zeros((4, shape[1] + 8), dtype=np.uint32)
    )

    # add amp 33 ref pixel array
    amp33_size = (n_groups, 4096, 128)
    wfi_image["amp33"] = kwargs.get(
        "amp33",
        u.Quantity(np.zeros(amp33_size, dtype=np.uint16), u.DN, dtype=np.uint16),
    )
    wfi_image["data"] = kwargs.get(
        "data",
        u.Quantity(
...
def mk_level3_mosaic(*, shape=(4088, 4088), n_images=2, filepath=None, **kwargs):
    ...
    if len(shape) > 2:
        ...
        warnings.warn(
            f"{MESSAGE} assuming the first entry is n_images followed by y, x. The"
            " remaining is thrown out!",
            UserWarning,
        )
    ...
    wfi_mosaic["data"] = kwargs.get(
        "data",
        u.Quantity(
            np.zeros(shape, dtype=np.float32), u.electron / u.s, dtype=np.float32
        ),
    )
    wfi_mosaic["err"] = kwargs.get(
        "err",
        u.Quantity(
            np.zeros(shape, dtype=np.float32), u.electron / u.s, dtype=np.float32
        ),
...
def mk_msos_stack(*, shape=(4096, 4096), filepath=None, **kwargs):
    ...
    if len(shape) > 2:
        ...
        warnings.warn(
            f"{MESSAGE} assuming the the first two entries are y, x. The remaining is"
            " thrown out!",
            UserWarning,
        )
    ...
    msos_stack["uncertainty"] = kwargs.get(
...
def mk_ramp(*, shape=(8, 4096, 4096), filepath=None, **kwargs):
    ...
    ramp["border_ref_pix_left"] = kwargs.get(
        "border_ref_pix_left",
        u.Quantity(
            np.zeros((shape[0], shape[1], 4), dtype=np.float32), u.DN, dtype=np.float32
        ),
    )
    ...
    ramp["border_ref_pix_top"] = kwargs.get(
        "border_ref_pix_top",
        u.Quantity(
            np.zeros((shape[0], 4, shape[2]), dtype=np.float32), u.DN, dtype=np.float32
        ),
    )
    ...
    # and their dq arrays
    ramp["dq_border_ref_pix_left"] = kwargs.get(
        "dq_border_ref_pix_left", np.zeros((shape[1], 4), dtype=np.uint32)
    )
    ramp["dq_border_ref_pix_right"] = kwargs.get(
        "dq_border_ref_pix_right", np.zeros((shape[1], 4), dtype=np.uint32)
    )
    ramp["dq_border_ref_pix_top"] = kwargs.get(
        "dq_border_ref_pix_top", np.zeros((4, shape[2]), dtype=np.uint32)
    )
    ramp["dq_border_ref_pix_bottom"] = kwargs.get(
        "dq_border_ref_pix_bottom", np.zeros((4, shape[2]), dtype=np.uint32)
    )

    # add amp 33 ref pixel array
    ramp["amp33"] = kwargs.get(
        "amp33",
        u.Quantity(
            np.zeros((shape[0], shape[1], 128), dtype=np.uint16), u.DN, dtype=np.uint16
        ),
    )

    ramp["data"] = kwargs.get(
        "data",
        u.Quantity(np.full(shape, 1.0, dtype=np.float32), u.DN, dtype=np.float32),
    )
...
def mk_ramp_fit_output(*, shape=(8, 4096, 4096), filepath=None, **kwargs):
    ...
    rampfitoutput["yint"] = kwargs.get(
        "yint",
        u.Quantity(np.zeros(shape, dtype=np.float32), u.electron, dtype=np.float32),
    )
    rampfitoutput["sigyint"] = kwargs.get(
        "sigyint",
        u.Quantity(np.zeros(shape, dtype=np.float32), u.electron, dtype=np.float32),
    )
    rampfitoutput["pedestal"] = kwargs.get(
        "pedestal",
        u.Quantity(np.zeros(shape[1:], dtype=np.float32), u.electron, dtype=np.float32),
    )
    rampfitoutput["weights"] = kwargs.get("weights", np.zeros(shape, dtype=np.float32))
    rampfitoutput["crmag"] = kwargs.get(
        "crmag",
        u.Quantity(np.zeros(shape, dtype=np.float32), u.electron, dtype=np.float32),
    )
...
def mk_rampfitoutput(**kwargs):
    warnings.warn(
        "mk_rampfitoutput is deprecated. Use mk_rampfit_output instead.",
        DeprecationWarning,
    )
...
def mk_associations(*, shape=(2, 3, 1), filepath=None, **kwargs):
    ...
    associations["degraded_status"] = kwargs.get(
    ...
    if "products" in kwargs:
        ...
        for product_idx, members in enumerate(shape):
            ...
            associations["products"].append(
                {"name": f"product{product_idx}", "members": members_lst}
            )
...
def mk_guidewindow(*, shape=(2, 8, 16, 32, 32), filepath=None, **kwargs):
    ...
    guidewindow["pedestal_frames"] = kwargs.get(
        "pedestal_frames",
        u.Quantity(np.zeros(shape, dtype=np.uint16), u.DN, dtype=np.uint16),
    )
    guidewindow["signal_frames"] = kwargs.get(
        "signal_frames",
        u.Quantity(np.zeros(shape, dtype=np.uint16), u.DN, dtype=np.uint16),
    )
    guidewindow["amp33"] = kwargs.get(
        "amp33", u.Quantity(np.zeros(shape, dtype=np.uint16), u.DN, dtype=np.uint16)
    )
