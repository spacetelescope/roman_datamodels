...
def mk_level1_science_raw(*, shape=(8, 4096, 4096), filepath=None, **kwargs):
    ...
    if len(shape) != 3:
        shape = (8, 4096, 4096)
        warnings.warn("Input shape must be 3D. Defaulting to (8, 4096, 4096)", stacklevel=2)
...
def mk_guidewindow(*, shape=(2, 8, 16, 32, 32), filepath=None, **kwargs):
    ...
    if len(shape) != 5:
        shape = (2, 8, 16, 32, 32)
        warnings.warn("Input shape must be 5D. Defaulting to (2, 8, 16, 32, 32)", stacklevel=2)
