...
def mk_level2_image(shape=None, n_ints=None, filepath=None):
    ...
    wfi_image['border_ref_pix_left'] = u.Quantity(np.zeros(shape, dtype=np.float32),
                                         ru.DN, dtype=np.float32)
    wfi_image['border_ref_pix_right'] = u.Quantity(np.zeros(shape, dtype=np.float32),
                                         ru.DN, dtype=np.float32)
    wfi_image['border_ref_pix_top'] = u.Quantity(np.zeros(shape, dtype=np.float32),
                                         ru.DN, dtype=np.float32)
    wfi_image['border_ref_pix_bottom'] = u.Quantity(np.zeros(shape, dtype=np.float32),
                                         ru.DN, dtype=np.float32)
...
def mk_ramp(shape=None, n_ints=None, filepath=None):
    ...
    ramp['amp33'] = u.Quantity(np.full(shape, 1.0, dtype=np.float32), ru.DN, dtype=np.float32)
...
def mk_rampfitoutput(shape=None, filepath=None):
    ...
    #rampfitoutput['crmag'] = u.Magnitude(np.zeros(shape, dtype=np.float32) * u.ct/u.s,
    #                                     dtype=np.float32)
...
def mk_guidewindow(shape=None, filepath=None):
    ...
    guidewindow['pedestal_frames'] = u.Quantity(np.zeros(shape, dtype=np.uint16),
                                         ru.DN, dtype=np.uint16)
#    guidewindow['pedestal_frames'] = u.Magnitude(np.zeros(shape, dtype=np.uint16) * u.ct/u.s,
#                                                 dtype=np.uint16)
