...
def mk_cal_logs():
    ...
    return stnode.CalLogs(
        [
            ...
            "2021-11-15T10:22.55.55Z :: RampFittingStep :: WARNING :: "
            "Wow, lots of Cosmic Rays detected",
        ]
    )
...
def mk_level2_image(shape=None, n_ints=None, filepath=None):
    ...
    wfi_image["border_ref_pix_left"] = np.zeros(
        (n_ints, shape[0] + 8, 4), dtype=np.float32
    )
    wfi_image["border_ref_pix_right"] = np.zeros(
        (n_ints, shape[0] + 8, 4), dtype=np.float32
    )
    wfi_image["border_ref_pix_top"] = np.zeros(
        (n_ints, 4, shape[1] + 8), dtype=np.float32
    )
    wfi_image["border_ref_pix_bottom"] = np.zeros(
        (n_ints, 4, shape[1] + 8), dtype=np.float32
    )
...
def mk_linearity(shape=None, filepath=None):
    """
    Create a dummy Linearity instance (or file) with arrays and valid values
    for attributes required by the schema.
...
def mk_pixelarea(shape=None, filepath=None):
    """
    Create a dummy Pixelarea instance (or file) with arrays and valid values
    for attributes required by the schema.
...
def mk_wfi_img_photom(filepath=None):
    """
    Create a dummy WFI Img Photom instance (or file) with dictionary and valid values
    for attributes required by the schema.
...
def mk_readnoise(shape=None, filepath=None):
    """
    Create a dummy Readnoise instance (or file) with arrays and valid values
    for attributes required by the schema.
...
def mk_ramp(shape=None, n_ints=None, filepath=None):
    ...
    # add border reference pixel arrays
    ramp["border_ref_pix_left"] = np.zeros((shape[0], shape[1], 4), dtype=np.float32)
    ramp["border_ref_pix_right"] = np.zeros((shape[0], shape[1], 4), dtype=np.float32)
    ramp["border_ref_pix_top"] = np.zeros((shape[0], 4, shape[2]), dtype=np.float32)
    ramp["border_ref_pix_bottom"] = np.zeros((shape[0], 4, shape[2]), dtype=np.float32)
...
def mk_rampfitoutput(shape=None, filepath=None):
    """
    Create a dummy Rampfit Output instance (or file) with arrays and valid values
    for attributes required by the schema.
...
def mk_guidewindow(shape=None, filepath=None):
    ...
    guidewindow["meta"]["gw_window_xstop"] = (
        guidewindow["meta"]["gw_window_xstart"] + 16.0
    )
    guidewindow["meta"]["gw_window_ystop"] = (
        guidewindow["meta"]["gw_window_ystart"] + 16.0
    )
...
def mk_superbias(shape=None, filepath=None):
    """
    Create a dummy Superbias instance (or file) with arrays and valid values
    for attributes required by the schema.
