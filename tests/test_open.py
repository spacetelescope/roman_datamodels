...
from .conftest import datamodel_types
...
def test_node_round_trip(tmp_path, node):
    file_path = tmp_path / "test.asdf"

    # Create/return a node and write it to disk, then check if the node round trips
    node_instance = node.create_fake_data()
    asdf.AsdfFile({"roman": node_instance}).write_to(file_path)
    with asdf.open(file_path) as af:
        assert_node_equal(af.tree["roman"], node_instance)


def test_opening_model(tmp_path, node):
    file_path = tmp_path / "test.asdf"

    # Create a node and write it to disk
    node_instance = node.create_fake_data()
    if hasattr(node_instance, "meta") and hasattr(node_instance.meta, "filename"):
        node_instance.meta.filename = type(node_instance.meta.filename)(file_path.name)
    asdf.AsdfFile({"roman": node_instance}).write_to(file_path)

    with datamodels.open(file_path) as model:
        # Check that the model is the correct type
        assert isinstance(model, datamodels.DataModel.datamodel_type(node))
...
@pytest.mark.parametrize(
    ...
    [
        model._node_type
        for model in datamodel_types(datamodels.DataModel)
        if ("Ref" not in model.__name__ and "Associations" not in model.__name__)
    ],
)
def test_filename_matches_meta(tmp_path, node):
    save_path = tmp_path / "test_filename.asdf"
    open_path = tmp_path / "test_filename_read.asdf"

    # Create a node and write it to disk
    gen_model = node.create_fake_data()
