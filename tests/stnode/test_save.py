...
@pytest.fixture(scope="session", params=REGISTRY.tag)
...
def node_cls(node_tag):
    return REGISTRY.tag.node[node_tag]
...
@pytest.mark.parametrize("manifest_uri", REGISTRY.manifest)
def test_registry_manifest_to_tags_is_disjoint(manifest_uri):
    """Test that the tags registered to each manifest are unique"""
    for uri, tags in REGISTRY.manifest.tag.items():
        if uri != manifest_uri:
            assert set(tags) & set(REGISTRY.manifest.tag[manifest_uri]) == set()
        else:
            assert len(set(REGISTRY.manifest.tag[manifest_uri])) == len(REGISTRY.manifest.tag[manifest_uri])

        for tag in tags:
            assert tag in REGISTRY.tag


def test_all_tags_in_manifest_to_tags_registry():
    assert sum(len(tags) for tags in REGISTRY.manifest.tag.values()) == len(REGISTRY.tag)
    assert len(REGISTRY.tag.node) == len(REGISTRY.tag.manifest) == len(REGISTRY.tag.schema) == len(REGISTRY.tag)
...
def test_history(tmp_path, node_tag, node_instance):
    ...
    assert REGISTRY.tag.manifest[node_tag] in datamodel_uris
    ...
    assert REGISTRY.tag.manifest[node_tag] == datamodel_uris[0]
